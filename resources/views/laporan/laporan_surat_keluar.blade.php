...
<!-- FILTER DATA -->
<form action="{{url('/laporan_surat_keluar/filter_periode')}}" method="GET">
<div class="modal fade" id="modalFilterSuratKeluar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<form action="{{url('/laporan_surat_keluar/filter_kode_surat')}}" method="GET">
<div class="modal fade" id="modalFilterKodeSuratKeluar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<table width="100%">
  <tr>
    <td width="20%">
       <button type="button" title="Filter Periode"  class="btn btn-success" data-toggle="modal" data-target="#modalFilterSuratKeluar"><span class="fa fa-search"></span>
                    Filter Periode
      </button>
      <button type="button" title="Filter Kode Surat"  class="btn btn-warning" data-toggle="modal" data-target="#modalFilterKodeSuratKeluar"><span class="fa fa-search"></span>
                    Filter Kode Surat
        </button>
        <?php
	...
	<a href="{{url('/laporan_surat_keluar/filter_periode/cetak/'.$awal.'/'.$akhir)}}" class="btn btn-primary" target="blank" data-toggle="modal" title="Cetak PDF">Cetak PDF</a>
	<br><br>
    ...
    $periode=" Periode ".Carbon\Carbon::parse($awal)->formatLocalized('%d %B %Y'). " Sampai ".Carbon\Carbon::parse($akhir)->formatLocalized('%d %B %Y');
      ...
      ?>
        ...
        <a href="{{url('/laporan_surat_keluar/filter_kode_surat/cetak/'.$txtKodeSurat)}}" class="btn btn-primary" target="blank" data-toggle="modal" title="Cetak PDF">Cetak PDF</a>
    <?php
  }else{
      ?>
      	<a href="{{url('/laporan_surat_keluar/filter_periode/cetak')}}" class="btn btn-primary" target="blank" data-toggle="modal" title="Cetak PDF">Cetak PDF</a>
      <?php
      }
    ?>
</p>
...
<!-- END -->
<table border="1" width="100%" class="table table-bordered table-hover" id="dataLaporanSuratKeluar">
	<thead>
		<tr>
			<th width="3%">No</th>
    ...
    <tr>
      ...
      <td><a href="{{url('cetak_surat_keluar/'.$p->id)}}" class="btn btn-warning" target="blank" data-toggle="modal" title="Cetak"><i class="fa fa-print" aria-hidden="true"></i></a></td>
    </tr>
    @endforeach
</tbody>

</table>
<!-- Jumlah Surat : <span style="background-color: red;padding: 4px;color: #fff;border-radius: 4px;">{{$jumlahSuratKeluar}}</span> -->
</div>
...
$(document).ready(function() {
    var t = $('#dataLaporanSuratKeluar').DataTable( {
        "columnDefs": [ {
            "searchable": false,
            "orderable": false,
            "targets": 0,
